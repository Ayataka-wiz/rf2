...
def main():
    ...
    if choice == 'グラフ表示':
        ...
        if 'df' in st.session_state:
            ...
            #セレクトボックスの表示
            act = df.columns.to_list()
            cho = st.sidebar.selectbox("データの選択", act)

            # グラフの表示
            #act
            st_display_graph(df,x_col = cho)
    ...
    if choice == '学習と検証':
        ...
        if 'df' in st.session_state:
            ...
            #深さを指定
            su = st.sidebar.number_input("決定木の深さ",1,10,1)

            # 決定木による予測
            clf, train_pred, train_scores = ml_dtree(train_X, train_Y, su)
            
            # 正解率を出力
            st.subheader("正解率：{}".format(train_scores))
    ...
    if choice == 'About':
        ...
        image = Image.open('stpy.png')
        st.image(image)

        st.markdown("Built by [Ayataka]")
        st.text("Thank you for using the app")

        st.markdown("For More Information check out   (https://note.com/)")
